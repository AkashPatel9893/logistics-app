...
| 13  | **The Squish Reflex**         | `scale: 0.96` press feedback on _every_ touchable - including full-width list rows - or `TouchableOpacity`'s washed-out flash                     | Rows highlight (background change); buttons scale slightly or dim; `Pressable` with per-role feedback. Never `TouchableOpacity`                                                                                               |
